<template>
  ...
  export default defineComponent({
    ...
    components: { Navbar },
    setup() {
      const shipments = ref(Array.from({ length: 100 }, (v, i) => ({
        id: i + 1,
        shippingDate: '',
        deliveryDate: '',
      ...
      const filteredShipments = computed(() => {
        ...
        // Filtrado por fecha de envío
        if (shippingDate.value) {
          filtered = filtered.filter(shipment => shipment.shippingDate === shippingDate.value);
        }
  
        // Filtrado por fecha de entrega
        if (deliveryDate.value) {
          filtered = filtered.filter(shipment => shipment.deliveryDate === deliveryDate.value);
        }
  
        // Filtrado por confirmación
        if (confirmationFilter.value) {
          filtered = filtered.filter(shipment => shipment.confirmation === confirmationFilter.value);
        }
      ...
      onMounted(() => {
        // Cualquier inicialización adicional se puede realizar aquí
      });
